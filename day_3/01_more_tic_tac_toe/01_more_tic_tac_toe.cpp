...
public:
    ...
    Grid(int x, int y); // this is the class constructor that is going to run when we define a new Grid object
...
void Grid::initialiseGrid(int x_dim, int y_dim){
...
bool Grid::checkInput(int x, int y){
...
void Grid::placeMarker(int x, int y, int marker){
    grid[y-1][x-1] = marker;
}
...
// we are having a "&" in front of the variable, means we are passing a reference of the vector, instead of passing a copy of it, so changes made here is reflectd in main()
void Player::playersMove(Grid &grid){
    int x, y;
    while(true){
